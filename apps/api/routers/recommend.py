...
def _score_products(products: Iterable[Product], goal: str, constraints: Dict) -> List[ScoredProduct]:
    ...
    for product in products:
        ...
        scored.append(ScoredProduct(product=product, score=score, rationale=rationale))
...
def recommend_products(request: RecommendRequest) -> SearchResponse:
    ...
    filtered = filter_products(catalog, filters=constraints)
    scored = _score_products(filtered, request.goal, constraints)
    ...
    debug = {"scored": len(scored), "after_diversity": len(diversified)}
