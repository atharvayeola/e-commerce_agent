...
def test_agent_chat_handles_unknown_query_with_follow_up():
    payload = {"message": "blargle zoot pants"}
    r = client.post("/agent/chat", json=payload)
    assert r.status_code == 200
    data = r.json()
    assert data["intent"] == "text_recommendation"
    assert len(data.get("products", [])) > 0
    assert data.get("follow_up_question") == "What budget should I keep in mind?"
